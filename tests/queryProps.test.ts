...
describe('QueryProps', () => {
  it('sets various query properties', () => {
    ...
    props
      ...
      .rangeValues([{ name: 'test', value: 1, range: [0, 10] }])
    ...
    expect((q as any).multipatchOption).toBe('xyFootprint')
    ...
    expect((q as any).quantizationParameters).toEqual({ tolerance: 1 })
    expect((q as any).rangeValues).toEqual([{ name: 'test', value: 1, range: [0, 10] }])
