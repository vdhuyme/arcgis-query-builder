...
export class QueryBuilder {
  ...
  where(
    ...
    if (typeof column === "function") {
      ...
      this._where
        .and()
        .where(column as string, operator as ComparisonOperator, value);
    }
  ...
  private _pushClause(clause: string) {
    this._currentOp === "OR"
      ? this._where.or().pushRaw(clause)
      : this._where.and().pushRaw(clause);
  }
  ...
  stats(
    statistics: QueryStatisticDefinitionInput | QueryStatisticDefinitionInput[]
  ) {
  ...
  whereBusinessHours(column: string) {
    this._pushClause(
      `EXTRACT(HOUR FROM ${column}) >= 9 AND EXTRACT(HOUR FROM ${column}) < 17`
    );
    this._updateWhereClause();
    return this;
  }
