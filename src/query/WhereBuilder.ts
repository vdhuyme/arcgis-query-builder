...
export class WhereBuilder {
  ...
  whereBetween(
    ...
    return this.pushRaw(
      `${column} BETWEEN ${formatValue(min)} AND ${formatValue(max)}`
    );
  }
  ...
  whereNotBetween(
    ...
    return this.pushRaw(
      `${column} NOT BETWEEN ${formatValue(min)} AND ${formatValue(max)}`
    );
  }
  ...
  whereDate(column: string, operator: ComparisonOperator, date: string | Date) {
    return this.pushRaw(
      `${column} ${operator} ${formatDateValue(date, "date")}`
    );
  }
  ...
  whereMonth(
    ...
    if (operator === "IN" || operator === "NOT IN") {
      ...
      return this.pushRaw(
        `EXTRACT(MONTH FROM ${column}) ${operator} (${values})`
      );
    }
  ...
  whereDayOfWeek(
    ...
    if (operator === "IN" || operator === "NOT IN") {
      const values = Array.isArray(dayOfWeek)
        ? dayOfWeek.join(", ")
        : dayOfWeek;
      return this.pushRaw(
        `EXTRACT(DOW FROM ${column}) ${operator} (${values})`
      );
    }
